...
k=100
rst_dir="../../../../md_3/linked/US_1" #dont include US_$i dir 

#make directories and mdin files
for i in 10 46 106;do
mkdir -p US_$i
cd US_$i
echo "Prod for US_$i
 &cntrl
...
#SBATCH --job-name=3.4US${i}l # Name to show in the job queue 
...
mv -n dihed_$i.dat dihed_$i.0.dat
mv -n dihed_$i.histo dihed_$i.0.histo  

pmemd.cuda -O -i US_$i.mdin -o US_$i.1.mdout -p $parm -c ${rst_dir}/US_$i/US_$i.20.rst7 -r US_$i.1.rst7 -x US_$i.1.nc -inf US_$i.1.mdinfo
