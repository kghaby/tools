...
fl1[0]="rmsd_l_refx_lig_FE.dat" #l
fl1[1]="rmsd_u_refx_lig_FE.dat" #u
fl2[0]="rmsd_l_refl_lig_FE.dat" #l
fl2[1]="rmsd_u_refl_lig_FE.dat" #u
...
gnuplot -persist << EOF
    ...
    set key t r
    set key spacing 0.9
    set title 'Free energy surfaces of Acrylamide RMSDs'

    set yrange [0:$ymax1]
    set ytics 0, ($yticmax1 * 0.1), $yticmax1
    #set ytics offset 0,0.5
    set ylabel 'dG (kcal/mol) from xtal state'
    set format x ""
    @TMARGIN; @SIDEMARGIN
    plot \
    ...
    #set ytics offset 0,-0.5
    set xlabel 'RMSD ({\305})'
    set ylabel 'dG (kcal/mol) from linked state'
    set format x
    @BMARGIN; @SIDEMARGIN
    plot \
    "${fl2[0]}"   u (0.1 * column(1)):2 every 10 w l lc rgb "slateblue1" title "L",\
    "${fl2[1]}"   u (0.1 * column(1)):2 every 10 w l lc rgb "dark-orange" title "U"
