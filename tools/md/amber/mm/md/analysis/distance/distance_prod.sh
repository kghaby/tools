...
maincommand="distance $mask out distance.dat noimage"
...
EOF
cpptraj -i xtal.traj
xtalval=$(tail -1 distance.dat | awk '{print $2}')

#linked
set -e; cd ../../linked/prod; set +e

cat >distance.traj <<EOF
parm $parm_l
EOF
...
cpptraj -i distance.traj
../../../../../code/histogram -i distance.dat -col 2  -o distance.histo -int .1
...
fl1[0]="../../linked/prod/distance.dat"
fl1[1]="../../unlinked/prod/distance.dat"
fl2[0]="../../linked/prod/distance.histo"
fl2[1]="../../unlinked/prod/distance.histo"
